	...
	for (int i = -1; i <= 1; i++)
	{
		for (int j = -1; j <= 1; j++)
		{
			if (field [pos_y+i][pos_x +j] == opponent) //check if you can find an opponents stone in a neighboring field
			{
				...
				while ((pos_y+goto_Y) > 0 && (pos_y+goto_Y)<8
						&& (pos_x+goto_X) > 0 && (pos_x+goto_X)<8) //durchläuft das ganze Feld
				{
 ...
 * Spieler keinen Zug machen können ist der andere wieder dran.
 *
